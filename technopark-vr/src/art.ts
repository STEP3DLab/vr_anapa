...
export function createArt(){
 ...
 return {root,update(t:number){core.rotation.set(t*.2,t*.3,0);core.scale.setScalar(1+Math.sin(t)*.08);for(let i=0;i<56;i++){const u=i/56*Math.PI*2,a=u+t*.17,r=1.65+.25*Math.sin(u*3+t*.5);dummy.position.set(Math.cos(a)*r,3.7+Math.sin(u*2+t*.2)*1.65,Math.sin(a)*r);dummy.rotation.set(u*2+t*.2,-a,u);dummy.updateMatrix();petals.setMatrixAt(i,dummy.matrix);}petals.instanceMatrix.needsUpdate=true;orbits.forEach((o,i)=>{o.rotation.y=t*(.08+i*.025)+i;o.rotation.z=t*.06+i;});dust.rotation.y=t*.025;},dispose(){[baseGeo,inlayGeo,coreGeo,petalGeo,orbitGeo,particlesGeo].forEach(g=>g.dispose());[silver,glow,dark,particleMat].forEach(m=>m.dispose());}};
}
