...
export default function Experience(){
 ...
 return <div className={`experience ${hub?'is-hub':'is-game'}${clean?' clean':''}${xr?' xr-active':''}`} onClick={e=>{const target=e.target as HTMLElement;if(e.detail>0&&!target.closest('dialog'))target.closest('button')?.blur();}}>
  ...
  {menu&&<><button className="menu-scrim" aria-label="Закрыть меню" onClick={()=>setMenu(false)}/><aside className="main-menu" aria-label="Главное меню">
   <div className="menu-head"><div><span>ТЕХНОПАРК РГСУ</span><h2>Главное меню</h2></div><button className="menu-close" aria-label="Закрыть меню" onClick={()=>setMenu(false)}>×</button></div>
  ...
  <footer className="bottom-dock">
   ...
   <div className="vr-entry"><button disabled={entering} onClick={enterVR}>{entering?'Подключение…':xr?'Выйти из VR':'◉ Войти в VR'}</button><span>{supported===false?'Для VR откройте ссылку в браузере очков':supported?'WEBXR ГОТОВ · ВЕРСИЯ 10.1':'ПРОВЕРКА WEBXR · ВЕРСИЯ 10.1'}</span></div>
  </footer>
