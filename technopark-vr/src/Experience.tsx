...
export default function Experience(){
 ...
 function toggleMenu(){if(!menu&&!hub&&!paused)game.current?.setPaused(true);setMenu(!menu);}
 function continueFromMenu(){game.current?.resume();setMenu(false);}
 ...
 return <div className={`experience ${hub?'is-hub':'is-game'}${clean?' clean':''}${xr?' xr-active':''}`} onClick={e=>{const target=e.target as HTMLElement;if(e.detail>0&&!target.closest('dialog'))target.closest('button')?.blur();}}>
  ...
  <header className="game-header">
   ...
   <div className="header-tools"><span className="live-badge">ВЕРСИЯ 10.2</span><button className="utility-control" aria-pressed={muted} onClick={()=>setMuted(game.current?.toggleSound()??false)}>{muted?'Звук выкл.':'♪ Звук вкл.'}</button><button onClick={openHelp}>Как играть ?</button><button className="utility-control" aria-pressed={presentation} onClick={()=>game.current?.setPresentation(!presentation)}>{presentation?'Показ ∞ включён':'Режим показа'}</button><button className="clean-toggle utility-control" aria-pressed={clean} onClick={()=>setClean(!clean)}>{clean?'Вернуть интерфейс':'Чистый вид ↗'}</button><button className="menu-toggle" aria-expanded={menu} onClick={toggleMenu}>{menu?'× Закрыть':'☰ Меню'}</button></div>
  </header>
  {menu&&<><button className="menu-scrim" aria-label="Закрыть меню" onClick={()=>setMenu(false)}/><aside className="main-menu" aria-label="Главное меню">
   <div className="menu-head"><div><span>ТЕХНОПАРК РГСУ / VR</span><h2>Центр управления</h2><p>Выберите пространство, режим демонстрации или продолжите текущую миссию.</p></div><button className="menu-close" aria-label="Закрыть меню" onClick={()=>setMenu(false)}>×</button></div>
   ...
   <section className="menu-current"><div><span>СЕЙЧАС</span><strong>{title}</strong><small>{sceneMeta.num} / {sceneMeta.tag}</small></div><div><span>ЦЕЛЬ</span><strong>{sceneMeta.objective}</strong><small>{sceneMeta.control}</small></div></section>
   <section className="menu-section"><div className="menu-section-title"><h3>Пространства</h3><span>4 СЦЕНЫ · ОДНА VR-СЕССИЯ</span></div><div className="menu-scenes">{sceneItems.map(item=><button key={item.id} className={scene===item.id?'active':''} onClick={()=>switchScene(item.id)}><span className="menu-num">{item.num}</span><span><small>{item.kicker}</small><strong>{item.name}</strong><em>{item.desc}</em></span><b>{scene===item.id?'●':'↗'}</b></button>)}</div></section>
   ...
   <section className="menu-section"><div className="menu-section-title"><h3>Сеанс</h3><span>БЫСТРЫЕ ДЕЙСТВИЯ</span></div><div className="menu-actions">{!hub&&paused&&<button className="menu-continue" onClick={continueFromMenu}>▶ Продолжить миссию</button>}{!hub&&<button onClick={()=>{game.current?.resume();game.current?.restart();setMenu(false);}}>↻ Сбросить испытание</button>}{hub&&<button onClick={()=>{game.current?.welcome();setMenu(false);}}>✦ Повторить появление «Ядра»</button>}<button onClick={newVisitor}>Новый посетитель</button><a href="?gallery=1">3D-галерея ↗</a></div></section>
   ...
   <footer><span>ВЕРСИЯ 10.2</span><span>WEBXR / THREE.JS</span><span>QUEST READY</span></footer>
  </aside></>}
  {!clean&&<>
  ...
  <footer className="bottom-dock">
   ...
   <div className="vr-entry"><button disabled={entering} onClick={enterVR}>{entering?'Подключение…':xr?'Выйти из VR':'◉ Войти в VR'}</button><span>{supported===false?'Для VR откройте ссылку в браузере очков':supported?'WEBXR ГОТОВ · ВЕРСИЯ 10.2':'ПРОВЕРКА WEBXR · ВЕРСИЯ 10.2'}</span></div>
  </footer>
