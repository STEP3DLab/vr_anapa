...
try{
 ...
 await page.getByRole('button',{name:'☰ Меню',exact:true}).click();await page.waitForTimeout(120);assert.equal(await page.locator('.main-menu').count(),1);const desktopMenu=await page.locator('.main-menu').boundingBox();assert.ok(desktopMenu&&desktopMenu.x>=0&&desktopMenu.y>=0&&desktopMenu.x+desktopMenu.width<=1440&&desktopMenu.y+desktopMenu.height<=900);await page.screenshot({path:output+'/desktop-menu.png'});await page.getByRole('button',{name:'Закрыть меню',exact:true}).click();
