...
try{
 ...
 await page.getByRole('button',{name:'☰ Меню',exact:true}).click();await page.waitForTimeout(120);assert.equal(await page.locator('.main-menu').count(),1);const desktopMenu=await page.locator('.main-menu').boundingBox();assert.ok(desktopMenu&&desktopMenu.x>=0&&desktopMenu.y>=0&&desktopMenu.x+desktopMenu.width<=1440&&desktopMenu.y+desktopMenu.height<=900);assert.equal(await page.locator('.menu-live').count(),1);assert.equal(await page.locator('.menu-vr').count(),1);await page.screenshot({path:output+'/desktop-menu.png'});await page.keyboard.press('Escape');assert.equal(await page.locator('.main-menu').count(),0,'Escape closes menu without navigation');await page.keyboard.press('m');assert.equal(await page.locator('.main-menu').count(),1,'M opens command menu');await page.keyboard.press('m');assert.equal(await page.locator('.main-menu').count(),0,'M toggles command menu');
 ...
 for(const viewport of [{width:390,height:844},{width:844,height:390}]){
  ...
  await page.getByRole('button',{name:'☰ Меню',exact:true}).click();await page.waitForTimeout(100);const menuBox=await page.locator('.main-menu').boundingBox();assert.ok(menuBox&&menuBox.x>=0&&menuBox.y>=0&&menuBox.x+menuBox.width<=viewport.width+1&&menuBox.y+menuBox.height<=viewport.height+1,'Command menu must fit mobile viewport');await page.screenshot({path:output+'/mobile-'+viewport.width+'-menu.png'});await page.getByRole('button',{name:'Закрыть меню',exact:true}).click();
