...
try{
 ...
 if(await page.evaluate(()=>('serviceWorker' in navigator))){
  ...
  await page.goto(url+'assets/models/credits.html',{waitUntil:'networkidle'});assert.match(await page.locator('body').innerText(),/J-Toastie/);await page.context().setOffline(true);await page.goto(url+'index.html?diagnostics=1',{waitUntil:'domcontentloaded'});await page.waitForTimeout(800);
 ...
 for(const viewport of [{width:390,height:844},{width:844,height:390}]){
  await page.setViewportSize(viewport);await page.waitForTimeout(250);assert.equal(await page.locator('.portal-cards button').count(),3);if(viewport.width<681)assert.equal(await page.locator('.portal-cards small').first().isVisible(),true,'mission summaries stay visible on the portrait hub');await page.screenshot({path:output+'/mobile-'+viewport.width+'-hub.png'});
  ...
  await page.locator('.portal-cards button').filter({hasText:'Полигон Лосинка'}).click();await page.waitForTimeout(400);await page.screenshot({path:output+'/mobile-'+viewport.width+'-cargo.png'});await page.getByRole('button',{name:'Ручная стрела · C',exact:true}).click();await page.waitForTimeout(150);if(viewport.width<681)assert.equal(await page.locator('.status-instruction').isVisible(),true,'mission control hint remains visible on portrait mobile');assert.equal(await page.getByRole('button',{name:'Поднять захват',exact:true}).isVisible(),true);await page.screenshot({path:output+'/mobile-'+viewport.width+'-manual-arm.png'});
 ...
 // Render the manual handling stages using the same public axis controls as the visitor.
 await page.evaluate(()=>{const q=window.__inspection;q.go('cargo');q.renderer.setAnimationLoop(null);q.fade.visible=false;q.introRoot.visible=false;q.cargo.robot.position.set(-5,0,0);q.cargo.update(0,0,0,false);q.cargo.setManual(true);q.camera.position.set(1,4.5,5);q.camera.lookAt(-5,1,0);});
 for(const [name,goal] of [['manual-grip',{x:0,y:.84,z:-3}],['manual-transfer',{x:2.5,y:2.65,z:-.85}],['manual-platform',{x:0,y:1.79,z:1.85}]]){
  await page.evaluate(({name,goal})=>{const q=window.__inspection,c=q.cargo;const axis=(error,index)=>{for(let i=0;Math.abs(error())>.015&&i<1200;i++){const inputs=[0,0,0];inputs[index]=Math.max(-1,Math.min(1,error()*15));c.moveArm(1/60,...inputs);c.update(1/60,0,0,true);}};axis(()=>2.65-c.wrist.y,2);axis(()=>-Math.atan2(Math.sin(Math.atan2(goal.x,-(goal.z+.85))-Math.atan2(c.wrist.x,-(c.wrist.z+.85))),Math.cos(Math.atan2(goal.x,-(goal.z+.85))-Math.atan2(c.wrist.x,-(c.wrist.z+.85)))),1);axis(()=>Math.hypot(goal.x,goal.z+.85)-Math.hypot(c.wrist.x,c.wrist.z+.85),0);axis(()=>goal.y-c.wrist.y,2);if(name!=='manual-transfer'){c.interact();for(let i=0;i<40;i++)c.update(1/60,0,0,true);}q.scene.updateMatrixWorld(true);q.renderer.render(q.scene,q.camera);},{name,goal});
  await page.screenshot({path:output+'/'+name+'.png'});
 }
 assert.equal(await page.evaluate(()=>window.__inspection.cargo.loaded&&!window.__inspection.cargo.handLoaded),true,'manual placement rests the cargo on the platform');
 // Imported props retain their own geometry and materials in a real renderer.
 await page.evaluate(()=>{const q=window.__inspection;q.go('drones');q.advance(270);q.desktopGun.position.set(0,-.06,-1.22);q.desktopGun.rotation.set(.1,1.12,-.08);q.renderer.render(q.scene,q.camera);});await page.screenshot({path:output+'/shotgun-closeup.png'});
 await page.evaluate(()=>{const q=window.__inspection;q.desktopGun.visible=false;const d=q.drones[0];d.g.position.set(0,2.7,3.2);d.g.rotation.set(.08,-.32,0);q.renderer.render(q.scene,q.camera);});await page.screenshot({path:output+'/quadcopter-closeup.png'});
 await page.evaluate(()=>{const q=window.__inspection;q.desktopGun.rotation.set(0,0,0);q.desktopGun.position.set(.3,-.24,-.25);});
